/*
---------------------------------------------------------------
--          Enhancements on DEFAULT clause
---------------------------------------------------------------
*/
...
create table test_invalid_default (
  col1   number  default rownum     --> ** ERROR **
);

create table test_invalid_default (
  col1   number  default prior     --> ** ERROR **
);

create table test_invalid_default (
  col1   number  default level     --> ** ERROR **
);

/* a DEFAULT expression CANNOT contain references to other columns */
create table test_invalid_default (
  col1   number,
  col2   number  default col1     --> ** ERROR **
);

/* The expression can be of any form EXCEPT a scalar subquery expression */
create table test_invalid_default (
  col1   number  default (select 1 from dual)     --> ** ERROR **
);
