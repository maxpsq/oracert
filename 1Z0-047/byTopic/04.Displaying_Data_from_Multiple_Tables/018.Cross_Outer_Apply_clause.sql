...
SELECT D.DEPARTMENT_NAME, OA.LAST_NAME, OA.FIRST_NAME
  FROM HR.DEPARTMENTS D
    OUTER APPLY (SELECT * FROM HR.EMPLOYEES E
                  WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID) OA
 ; -- 122 ROWS
...
SELECT D.DEPARTMENT_NAME, CA.LAST_NAME, CA.FIRST_NAME
  FROM HR.DEPARTMENTS D
    CROSS APPLY (SELECT * FROM HR.EMPLOYEES E
                  WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID) CA
 ; -- 106 ROWS

-- ... THAT'S EQUIALENT TO ANSI SQL CROSS JOIN...
...
-- ... THAT'S EQUIVALENT TO ANSI SQL INNER JOIN.
SELECT DEPARTMENT_NAME, LAST_NAME, FIRST_NAME
  FROM HR.DEPARTMENTS D        
