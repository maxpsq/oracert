...
SELECT fact_1_id,
       fact_2_id,
       SUM(sales_value) AS sales_value,
       'Gr1 '||grouping(fact_1_id) as gr1,
       'Gr2 '||grouping(fact_2_id) as gr2,
       'Level '||(grouping(fact_1_id)+grouping(fact_2_id)) as grp_level1,
       'Level '||grouping_id(fact_1_id,fact_2_id) as grp_level2
FROM   dimension_tab
...
-- ** Limiting the number of subtotals generated by ROLLUP **
-- You can limit the number of subtotals produced just by grouping columns
-- in parentheses.
-- The following query will produce subtotals for sales_rep_id only, plus the
-- grand total.
select a.order_mode, a.order_status, a.sales_rep_id, a.order_id, sum(a.order_total)
from ( select order_mode, order_status, nvl(sales_rep_id,0) as sales_rep_id, order_id, order_total from oe.orders ) a
group by rollup(a.sales_rep_id, (a.order_mode, a.order_status, a.order_id) )
...
GROUPING SETS shows only super aggregate rows, and hides rows generated by the
"group by" clause
*/
